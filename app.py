...
def plaid_exchange():
    ...
    db.session.add(item)
    db.session.flush()  # get item.id before saving accounts
...
def plaid_sync():
    ...
    """
    # Optional: sync a specific bank only (from per-item Sync button)
    item_id = request.form.get("item_id", type=int)
    query = PlaidItem.query.filter_by(user_id=current_user.id)
    items = [query.get(item_id)] if item_id else query.all()
    items = [i for i in items if i]  # drop None if item_id wasn't found
    ...
    try:
        for item in items:
            # Map Plaid's account_id → human-readable name for this bank
            account_map = {a.plaid_account_id: a.name for a in item.accounts}

            # Fetch changes from Plaid (cursor = None on first run = full history)
            added, removed, next_cursor = sync_transactions(item)

            # Save new transactions (build_transactions_from_plaid handles dedup)
            ...
            # (e.g. a pending charge that never cleared)
            ...
            if removed_ids:
                deleted = Transaction.query.filter(
                    ...
                    Transaction.user_id == current_user.id  # safety: never delete another user's data
                ).delete(synchronize_session=False)
                total_removed += deleted

            # Advance the cursor so next sync is incremental (not full history again)
